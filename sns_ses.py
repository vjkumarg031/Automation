
//Amazon SES ---> Create identity ---> email address to register and verify it on email //
// SNS to SES //


import boto3

AWS_ACCESS_KEY_ID = "AKIAW6KT6TBGUO3PR5WZ"
AWS_SECRET_ACCESS_KEY = "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"
AWS_REGION = 'us-east-1'

def send_email_sns_ses(subject, message, recipient):
    sns = boto3.client('sns', aws_access_key_id=AWS_ACCESS_KEY_ID, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, region_name=AWS_REGION)
    ses = boto3.client('ses', aws_access_key_id=AWS_ACCESS_KEY_ID, aws_secret_access_key=AWS_SECRET_ACCESS_KEY, region_name=AWS_REGION)

    topic_response = sns.create_topic(Name='email-topic')
    topic_arn = topic_response['TopicArn']

    email_body = "This is a test email sent from Amazon SNS and SES"
    email_subject = "Test Email from SNS and SES"
    email_from = "kranthir67@gmail.com"

    ses_response = ses.send_email(
        Source=email_from,
        Destination={
            'ToAddresses': [
                recipient,
            ]
        },
        Message={
            'Subject': {
                'Data': subject
            },
            'Body': {
                'Text': {
                    'Data': message
                }
            }
        }
    )

    sns.publish(
        TopicArn=topic_arn,
        Message=message,
        Subject=subject
    )

    print("Email sent successfully!")

if __name__ == "__main__":
    send_email_sns_ses('Hello, This is a test email sent using Amazon SNS and SES', 'Test Email from SNS and SES',  'kranthir67@gmail.com')